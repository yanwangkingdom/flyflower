...
public class Copy_8_of_RTSoundEvent extends SoundEvent {
    long prectsmple=0;
...
//	System.out.println("audiostream是否是打开的"+this.isOpen());		
		...
		public int readSamples(byte[] buf, int offset, int length) {
			...
			*/
			...
			double startctime= Copy_8_of_RTSoundEvent.this.time;
			
			do{
				
				 if(alfilltotal==0){
					 needfilltime=autbeforect;
				 }
				...
				if(c==null){
					...
					double currentctime= Copy_8_of_RTSoundEvent.this.time;
				...
				//System.out.println("当前音频帧的pts是"+pktpts);
				 ...
				 needfilltime=  ((alfilltotal*1.0)/(sampleRate*channels*bytepersample))-currentctime;
				 
				if( needfilltime < autbeforect){
					needfillbyte=timeToByte(needfilltime);
				...
				// int audiosize=  this.aurecycleandtool.getSyncedAudioByGivenArray(audatabox, ((audatabox.connectorpts_blurwritensample*1.0/audatabox.connectorsrcsamplerate)-currentctime), sampledatapool);
				 ...
				 int audiosize=  this.aurecycleandtool.getSyncedAudioByGivenArray(audatabox, audiopts-currentctime, sampledatapool);
