...
/**
 * 这个类目前设置的这些参数 可以完美的实现音视频的同步
 * @author wang
 *
 */
public class Copy_9_of_RTSoundEvent extends SoundEvent {
    long prectsmple=0;
...
//	System.out.println("audiostream是否是打开的"+this.isOpen());		
		...
		public int readSamples(byte[] buf, int offset, int length) {
			...
			*/
			...
			double startctime= Copy_9_of_RTSoundEvent.this.time;
			...
			do{
				  ...
				  AudioDataBox databox=null;
						...
						//	this.samplesperframe=audatabox.connectorsrcsamplesperframe;
						    ...
						    alfilltotal= -timeToByte(0.5);
						    
						   
				       }
				
					 
					    currentctime= Copy_9_of_RTSoundEvent.this.time;
	...
	System.out.println("计算出的需要填充的字节数"+needfillbyte);					
				...
				// int audiosize=  this.aurecycleandtool.getSyncedAudioByGivenArray(audatabox, ((audatabox.connectorpts_blurwritensample*1.0/audatabox.connectorsrcsamplerate)-currentctime), sampledatapool);
				 ...
				 int audiosize=  this.aurecycleandtool.getSyncedAudioByGivenArray(databox, (audioptsdrift+internaltime)-currentctime, sampledatapool);
				 ...
				 audiopts=((databox.connectorpts_blurwritensample+databox.connectorsrcsamplesperframe)*1.0/databox.connectorsrcsamplerate)-(2.0*hw_buffer_size); 
