...
/**
 * 这个代码实现的是 有多少读多少 ，没有复杂的条件判断
 * @author wang
 *
 */
public class Copy_4_of_RTSoundEvent extends SoundEvent {
    long prectsmple=0;
  ...
  //  volatile boolean audioread;
	...
	public class RTAudioStream extends AudioStream{
		...
		public class AudioDataBox implements AudioConnector{
			...
			long inervaltime=currenttime-prewrite;
			prewrite=currenttime;
		while(true){		
				bufferlock.lock();
				...
				try{
					...
					float currentctime= Copy_4_of_RTSoundEvent.this.time;
			...
			System.out.println(audiodatabox.get_pts_Blurwritensample()+"当前帧超越了电影时间此帧作废 ");			
					...
					*/
					...
					if(ringbuf.writeAvailable()>= sampleToByte(RTAudioStream.this.samplesperframe)){
						...
						if(audiodatabuffer.array()!=null){
							int writeed=ringbuf.write(audiodatabuffer.array(), 0, audiodatabuffer.capacity(), false);
							System.out.println(byteToSample(writeed)+"个采样被成功被写入"+"距离上次的时间间隔是"+inervaltime);
							...
							int writeed=ringbuf.write(srcsample, 0, audiodatabuffer.capacity(), false);
						...
						try {
 System.out.println(audiodatabox.get_pts_Blurwritensample()+"当前帧没有丢帧但是写有效少对接体开始等待"+"距离上次的时间间隔是"+inervaltime);
							buffercondition.awaitNanos(1000000);
...
//	System.out.println(RTSoundEvent.this.time+"我来读数据了");		
	...
	//System.out.println("当前的电影时间采样"+currentctsamples);	
			...
			long time=currenttime-ntervaltime;
			ntervaltime=currenttime;
			...
			try{
				...
				if(readAvailable>0){
					if(readAvailable>=buf.length){
					readedsize=ringbuf.read(buf, 0, buf.length, false);
					preventSecondEat=false;
			System.out.println("已经读出来多少采样"+byteToSample(readedsize)+"距离上一次来过了多少毫秒"+time);		
								
					return readedsize;
					}else{
						readedsize=ringbuf.read(buf, 0, readAvailable, false);
			...
			System.out.println("已经读出来多少采样"+byteToSample(readedsize)+"距离上一次来过了多少毫秒"+time);		
					...
					System.out.println("读出了0个采样"+"距离上一次来过了多少毫秒"+time);
					return 0;
				}
